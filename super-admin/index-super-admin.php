...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar_personaje'])) {
    ...
    $codigo = trim($_POST['editar_codigo']); // Nuevo campo

    // Obtener la foto actual del personaje
    $sql_get_foto = $con->prepare("SELECT foto FROM personajes WHERE id_personaje = :id_personaje");
    $sql_get_foto->bindParam(':id_personaje', $id_personaje, PDO::PARAM_INT);
    $sql_get_foto->execute();
    $foto_actual = $sql_get_foto->fetchColumn();
    ...
    if (isset($_FILES['editar_foto']) && $_FILES['editar_foto']['error'] === UPLOAD_ERR_OK) {
        ...
        // Si no se sube una nueva foto, conservar la foto actual
        $foto_ruta = $foto_actual;
    }

    // Actualizar el personaje en la base de datos
    $sql_update = $con->prepare("UPDATE personajes SET nom_personaje = :nom_personaje, fuerza = :fuerza, codigo = :codigo, foto = :foto WHERE id_personaje = :id_personaje");
    $sql_update->bindParam(':nom_personaje', $nom_personaje, PDO::PARAM_STR);
    $sql_update->bindParam(':fuerza', $fuerza, PDO::PARAM_INT);
    $sql_update->bindParam(':codigo', $codigo, PDO::PARAM_STR);
...
<body onload="editar_personaje.editar_nom_personaje.focus()">
    <div class="header">
    ...
    <div id="crear-personaje-modal" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0, 0, 0, 0.9); padding: 20px; border-radius: 10px; width: 400px; z-index: 1000;">
        ...
        <form action="crear-personaje.php" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="codigo" id="codigo" placeholder="Código del Personaje" required>
                ...
                <?php foreach ($personajes as $personaje): ?>
                    <tr>
                        ...
                        <td class="action-buttons">
                            <button class="edit" onclick="abrirEditarModal(<?= $personaje['id_personaje'] ?>, '<?= htmlspecialchars($personaje['nom_personaje']) ?>', <?= $personaje['fuerza'] ?>, '<?= htmlspecialchars($personaje['codigo']) ?>')">Editar</button>
    ...
    <div id="editar-personaje-modal" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0, 0, 0, 0.9); padding: 20px; border-radius: 10px; width: 400px; z-index: 1000;">
        ...
        <form action="index-super-admin.php" method="POST" enctype="multipart/form-data" name="editar_personaje">
            ...
            <input type="text" name="editar_codigo" id="editar-codigo" required>
    ...
    <script>
        ...
        function abrirEditarModal(id, nombre, fuerza, codigo) {
