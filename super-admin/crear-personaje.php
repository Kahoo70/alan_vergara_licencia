<!-- filepath: c:\xampp\htdocs\dead_ahead\super-admin\crear-personaje.php -->
<?php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $codigo = trim($_POST['codigo']); // Nuevo campo
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($foto_tmp, $foto_destino)) {
            try {
                ...
                $sql = $con->prepare("INSERT INTO personajes (nom_personaje, fuerza, codigo, foto, creado_por, fecha_creado) VALUES (:nom_personaje, :fuerza, :codigo, :foto, :creado_por, :fecha_creado)");
                $sql->bindParam(':nom_personaje', $nom_personaje, PDO::PARAM_STR);
                $sql->bindParam(':fuerza', $fuerza, PDO::PARAM_INT);
                $sql->bindParam(':codigo', $codigo, PDO::PARAM_STR); // Nuevo campo
